...
export class FirmWeb3 {
  ...
  _handleRetries (callback, progress, fn) {
    return handleRetries(this._retry, callback, progress, fn);
  }
...
function handleRetries (retrySettings, callback, progress, fn) {
  const retry = (left) => {
    if (left <= 0) {
      const err = {
    ...
    fn(() => {
      setTimeout(() => retry(left - 1), retrySettings.timeout);
    });
  };

  retry(retrySettings.limit);
}
